...
//				mouseL=e.pageX;
		...
		$(window).mousemove(function(e){
			...
			if(!lock){
				...
				$(".roundies").each(function(i,o){
					var $o=$(o);
					$o.css({
						left:(parseFloat($o.css("left"))+((mouseL-e.pageX)*(Math.pow(ratio,$o.attr("layer"))))/1000)+"px",
						top:(parseFloat($o.css("top"))+((mouseR-e.pageY)*(Math.pow(ratio,$o.attr("layer"))))/1000)+"px",
						//webkitTransform:"rotate("+(((mouseR-(e.pageY+e.pageX)/2)*(Math.pow(ratio,$o.attr("layer"))))/1000)+"deg)"
					});
				});
				mouseL=e.pageX;
				mouseR=e.pageY;
	...
	function getCSS(){
		var size=getSize();
		var t=getTop();
		var rotate=getRand(0,maxRotate);
		return {
